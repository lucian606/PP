...
(define-struct flappy (x y v-y) #:transparent) ;
(define-struct pipe (x y) #:transparent);Coordonatele gapului unui pipe (colt stanga sus)
...
(define (get-initial-state)
  (define initial-bird (flappy bird-x bird-initial-y 0))
  ...
  (define initial-state
    (state initial-bird (cons initial-pipe '()) initial-variables 0 '()))
  initial-state)
...
(define (next-state-bird bird gravity)
 (struct-copy flappy bird [v-y (+ (flappy-v-y bird) gravity )] [y (+ (get-bird-y bird) (get-bird-v-y bird))]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy flappy bird [v-y (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (cond [(key=? pressed-key " ") (struct-copy state current-state [bird (next-state-bird-onspace (get-bird current-state) (get-variables-momentum (get-variables current-state)))])]
        [else current-state]))
;TODO 9
...
(define move-pipe
  (lambda (scroll-speed)
    (lambda (current-pipe)
    (struct-copy pipe current-pipe [x (- (get-pipe-x current-pipe) scroll-speed)]))))

(define (move-pipes pipes scroll-speed)
  (map (move-pipe scroll-speed) pipes))
...
(define (is-pipe-on-screen pipe)
  (let* [(x (get-pipe-x pipe))
        (corner (+ x pipe-width))]
        (>= corner 0))) 

(define (clean-pipes pipes)
  (filter is-pipe-on-screen pipes))
...
(define (add-more-pipes pipes)
 (if (< (length pipes) no-pipes)
  (let* [(furthest-pipe (car pipes) )
             (new-pipe (struct-copy pipe furthest-pipe [x (+ (get-pipe-x furthest-pipe) pipe-width pipe-gap)] [y (+ added-number (random random-threshold))]))]
         (cons new-pipe pipes))
      pipes))
...
(define (check-ground-collision bird)
 (let [(y (+ (get-bird-y bird) bird-height))]
   (>= y ground-y)))
...
(define (invalid-state? state)
    ...
    (or (check-ground-collision bird)
            (check-pipe-collisions  bird pipes))))
...
(define (check-pipe-collisions bird pipes)
 (if (null? pipes)
     ...
     (let*[(current-pipe (car pipes))
           (BX (flappy-x bird))
           (BY (get-bird-y bird))
           (PX (get-pipe-x current-pipe))
           (PY (pipe-y current-pipe))
           (BP1 (make-posn BX BY)) ;Bird Point 1
           (BP2 (make-posn (+ BX bird-width) (+ BY bird-height))) ;Bird Point 2
           (UPP1 (make-posn PX 0)) ;Upper Pipe Point 1
           (UPP2 (make-posn (+ PX pipe-width) PY)) ;Upper Pipe Point 2
           (LPP1 (make-posn PX (+ PY pipe-self-gap))) ;Lower Pipe Point 1
           (LPP2 (make-posn (+ PX pipe-width) scene-height)) ;Lower Pipe Point 2 
          ]
       (if (or (check-collision-rectangles BP1 BP2 UPP1 UPP2) (check-collision-rectangles BP1 BP2 LPP1 LPP2))
           #t
...
(define (next-state current-state)
  (let* [(gravity (get-variables-gravity (get-variables current-state)))
         (scroll-speed (get-variables-scroll-speed (get-variables current-state)))
        (next-bird (next-state-bird (get-bird current-state) gravity))
        (next-pipes (next-state-pipes (get-pipes current-state) scroll-speed))
        (next-score (+ 0.1 (get-score current-state)))]
   (struct-copy state current-state [bird next-bird] [pipes next-pipes] [score next-score])))
...
(define (draw-frame state)
  (let* [(bird (get-bird state))
         ...
         (score-image (score-to-image score))
         (pipes-placed (place-pipes pipes initial-scene))
         (score-placed (place-image score-image text-x text-y pipes-placed))
         (ground-placed (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2)) score-placed))]
    (place-image bird-image (+ (flappy-x bird) (quotient bird-width 2)) (+ (get-bird-y bird) (quotient bird-height 2)) ground-placed)))
; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipe pipe scene)
  (let* [(PipeY (pipe-y pipe))
         (Upper-Pipe-X (+ (get-pipe-x pipe) (quotient pipe-width 2)))
         (Upper-Pipe-Y (quotient PipeY 2))
         (Lower-Pipe-X Upper-Pipe-X)
         (Lower-Pipe-Y (quotient (+ scene-height PipeY pipe-self-gap) 2))
         (Upper-Pipe-Image (rectangle pipe-width PipeY "solid" "green"))
         (Lower-Pipe-Image (rectangle pipe-width (- scene-height (+ PipeY pipe-self-gap)) "solid" "green"))
         (Added-Upper (place-image Upper-Pipe-Image Upper-Pipe-X Upper-Pipe-Y scene))]
         (place-image Lower-Pipe-Image Lower-Pipe-X Lower-Pipe-Y Added-Upper)))
...
(define get-variables (lambda (state)
  (state-variables state)))
(define get-variables-gravity (lambda (variables)
  (variables-gravity variables)))
(define get-variables-momentum (lambda (variables)
  (variables-momentum variables)))
(define get-variables-scroll-speed (lambda (variables)
