...
(define (number-of-palindrome-bases n Bases)
  (if (null? Bases)
      0
      (if (palindrome? (num->base n (car Bases)))
          (+ 1 (number-of-palindrome-bases n (cdr Bases)))
          (number-of-palindrome-bases n (cdr Bases)))))

(define (first-b-pal start b)
  (if (>= (number-of-palindrome-bases (add1 start) '(2 3 4 5 6 7 8 9 10)) b)
      (add1 start)
...
(define (long-list-pal L)
  (if (palindrome? L)
      L
      (if (> (length (long-list-pal (take L (sub1 (length L)))))
             (length (long-list-pal (cdr L))))
          (long-list-pal (take L (sub1 (length L))))
          (long-list-pal (cdr L))
          ) 
      ))

(define (longest-palindrome n)
  (list->num (long-list-pal (num->base n 10)))
  )
