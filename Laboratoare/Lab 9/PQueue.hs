...
instance (Num a, Enum a, Show b) => Show (a->b) where
  show f = show (map f [1..10])

setIntersection :: Eq a => [a] -> [a] -> [a]
setIntersection a b = [x | x <- a, x `elem` b]

instance (Enum a, Num a, Eq b) => Eq (a->b) where
  f == g = (length (setIntersection (map f [1..100]) (map g [1..100]))) >= 10

instance Num Bool where
  val1 + val2 = val1 || val2
  val1 * val2 = val1 && val2

data Tree k = Nod k ([Tree k])

height (Nod _ []) = 0
height (Nod _ lst) =  1 + (maximum (map height lst))

leaves (Nod _ []) = 0
leaves (Nod _ lst) = (sum (map leaves lst))

data MultiMap k v = Mmap [(k,[v])] deriving (Eq, Show)

get_lst (Mmap l) = l;

ins key val (Mmap lst)
  | lst == [] = Mmap [(key, [val])] --Nu am cheia, fac eu pereche
  | key == (fst (head lst)) = Mmap ([(key,(snd (head lst)) ++ [val])] ++ (tail lst)) -- Am gasit cheie, actualizez valoarea
  | otherwise = Mmap ([(head lst)]  ++ (get_lst (ins key val (Mmap (tail lst))))) -- Apel recursiv daca nu gasesc nimic

look key (Mmap lst)
  | lst == [] = []
  | key == (fst (head lst)) = (snd (head lst))
  | otherwise = look key (Mmap (tail lst))

map_m f (Mmap lst)
  | lst == [] = Mmap []
  | otherwise = Mmap ([(fst (head lst) , map f (snd (head lst)))] ++ get_lst (map_m f (Mmap (tail lst))))

data HashMap k v = Hmap [(k,[v])] deriving (Eq, Show)
...
insH val (Hmap lst)
  | lst == [] = Hmap [(key, [val])] --Nu am cheia, fac eu pereche
  | key == (fst (head lst)) = Hmap ([(key,(snd (head lst)) ++ [val])] ++ (tail lst)) -- Am gasit cheie, actualizez valoarea
  | otherwise = Hmap ([(head lst)]  ++ (get_lstH (insH val (Hmap (tail lst))))) -- Apel recursiv daca nu gasesc nimic
  where key = (hash val)

map_H f (Hmap lst)
  | lst == [] = Hmap []
  | otherwise = Hmap ([(fst (head lst) , map f (snd (head lst)))] ++ get_lstH (map_H f (Hmap (tail lst))))

data Clist a = Clist [a] deriving (Eq, Show)

get (Clist lst) = head lst

next (Clist lst) = (Clist ((tail lst) ++ [head lst]))

data Ceva k = Carac k | Numar k | Pair k deriving (Eq, Show)

data Mlist k = Ml [Ceva k]
...
rareChars lst n
  | lst == [] = []
  | (snd (head lst)) < n = (fst (head lst)) : (rareChars (tail lst) n)
  | otherwise = (rareChars (tail lst) n)

insChar :: Char -> FreqTable -> FreqTable
insChar c lst
  | lst == [] = [(c, 1)]
  | (fst (head lst)) == c = (c, (snd (head lst)) + 1) : (tail lst)
  | otherwise = (head lst) : (insChar c (tail lst))

textToStr :: String -> String
textToStr s = filter (/=' ') s

textToTable :: String -> FreqTable
textToTable st = foldl insChar st []

--[(fst(head lst)),map f (snd (head lst))] ++ (get_lst (map_m f (Mmap (tail lst))))
check1 :: TestData
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert (priority, a) (LPQ l) =
      if (length l) == 0 then
        LPQ [(priority, a)]
      else if (priority > (fst (head l))) then
          (LPQ ((priority, a) : l))
          else (LPQ ((head l) : toList (insert (priority, a) (LPQ (tail l)))))                             
...
merge (Empty r1) (Empty r2) = (Empty r1)
merge (Empty r) (Node rang val left right) = (Node rang val left right)
merge (Node rang val left right) (Empty r) = (Node rang val left right)
merge (Node rang1 val1 left1 right1) (Node rang2 val2 left2 right2) = 
  if((fst val1) > (fst val2)) then       --daca prima prioritate e mai mare
    if(rank(merge right1 (Node rang2 val2 left2 right2)) < rank left1) then --daca arborele stang are rank mai mare las asa
      (Node (1+rank(merge right1 (Node rang2 val2 left2 right2))) val1 left1 (merge right1 (Node rang2 val2 left2 right2)))
      else  --fac switch intre subarbori ca stanga sa aibe rank mai mare
        (Node (1+rank(merge right1 (Node rang2 val2 left2 right2))) val1 (merge right1 (Node rang2 val2 left2 right2)) left1)
    else if((rank (merge right2 (Node rang1 val1 left1 right1))) < rank left2) then --daca a doua e mai mare si arborele stang are rank mai mare
      (Node (1+(rank (merge right2 (Node rang1 val1 left1 right1)))) val2 left2 (merge right2 (Node rang1 val1 left1 right1)))
      else  --fac schimbarea intre fii
        (Node (1+(rank (merge right2 (Node rang1 val1 left1 right1)))) val2 (merge right2 (Node rang1 val1 left1 right1)) left2)


inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty r) = []
inorder (Node rang val left right) =
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    isEmpty (Node rank val left right) = False
    
    insert (priority, val) (Empty _) = (Node 0 (priority, val) empty empty)
    insert (priority, val) (Node rank1 val1 left1 right1) = 
      (merge (Node rank1 val1 left1 right1) (insert (priority, val) (Empty 0)))

    top (Empty _) = Nothing
    top (Node rank val left right) = Just val

    pop (Empty _) = (Empty 0)
    pop (Node rank val left right) = (merge left right)
