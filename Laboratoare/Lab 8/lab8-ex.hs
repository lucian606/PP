...
normalizeV (V x y z) = 
  let
    norma = lengthV (V x y z)
  in
    V (x / norma) (y / norma) (z / norma) 
...
orthogonalV v  
  | v == [] = False
  | (tail v) == [] = False
  | (dotV (head v) (head (tail v))) == 0 = True
  | otherwise = orthogonalV (tail v)
...
data MyMap k v = Entry [(k, [v])] deriving Show

insertP key val (Entry lst)
  | lst == [] = Entry [(key, [val])]
  | (fst (head lst)) == key =  val : (snd (head lst))
  | otherwise = (Entry ((head lst) : (insertP key val (Entry (tail lst)))))

data SList a = EmptyList | AddFirst a (SList a) | AddSList (SList a) (SList a) deriving Show

emptySList :: SList a
emptySList = EmptyList

consElem :: a -> SList a -> SList a
consElem a lst = AddFirst a lst

consList :: SList a -> SList a -> SList a
consList lst1 lst2 = AddSList lst1 lst2

headSList :: SList a -> SList a
headSList EmptyList = EmptyList
headSList (AddFirst a lst) = AddFirst a EmptyList
headSList (AddSList lst1 lst2) = lst1
 
tailSList :: SList a -> SList a
tailSList EmptyList = EmptyList
tailSList (AddFirst a lst) = lst
tailSList (AddSList lst1 lst2) = lst2

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual EmptyList EmptyList = True
deepEqual (AddFirst a lst1) (AddFirst b lst2) = 
  if a == b then
    if deepEqual lst1 lst2 == True
      then True
      else False
      else False
deepEqual (AddSList lst1 lst2) (AddSList lst3 lst4) =
  if deepEqual lst1 lst3 == True then
    if deepEqual lst2 lst4 == True
      then True
      else False
      else False
deepEqual EmptyList _ = False
deepEqual (AddFirst a lst1) _ = False
deepEqual (AddSList lst1 lst2) _ = False 

flatten :: SList a -> [a]
flatten EmptyList = []
flatten (AddFirst a lst1) = [a] ++ flatten lst1
flatten (AddSList lst1 lst2) = flatten lst1 ++ flatten lst2
...
insertElem BSTNil  a = Tree a BSTNil BSTNil
insertElem (Tree b left right) a =
  if b > a then
    (Tree b (insertElem left a) right)
    else
      (Tree b left (insertElem right a)) 

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil a =
  Nothing

findElem (Tree b left right) a =
  if a == b then
    Just a
    else if b > a then
      findElem left a
...
subTree BSTNil a b = 
  Nothing
subTree (Tree c left right) a b =
  if ((findElem (Tree c left right) b) == Nothing) then
    Nothing
    else if ((findElem (Tree c left right) a) == Nothing) then
    Nothing
      else if ((c >= a && b >= c) || (c <= a && b <= c)) then
        Just (Tree c left right)
...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder (Tree a left right) = (inorder left) ++ (a : []) ++ (inorder right)
...
data TList = Nil | Cons Int TList deriving (Eq)
