...
myReverseAcc([],L,L).
myReverseAcc([H|T],A,R) :- myReverseAcc(T,[H|A],R).
...
extract([],_,_,[]).
extract([H|T],0,0,[H]).
extract([H|T],0,E,[H|ACC]) :- E1 is E-1, extract(T,0,E1,ACC).
extract([H|T],S,E,R) :- S1 is S-1, extract(T,S1,E,R).
...
factorial(0,1).
factorial(1,1).
factorial(N,F) :- M is N - 1, factorial(M,A), F is A * N.
...
sublista(_,[]).
sublista( [H|T], [H|TS]) :- sublista(T, TS).
