...
--Concatenez la finalul listei elementul curent
myCycle l = head l : myCycle ((tail l) ++ [head l]) 
{-
  2. (2p)
...
geometric  initial r = iterate (*r) initial
{-
  3. (2p)
...
randoms seed = 
	let
		...
		--urm = (next (snd generator))
		--nr = (fst urm)
	in
    (randoms1 seed generator)
		--(fromInteger (toInteger nr)) : (randoms nr)

randoms1 seed acc =
    let
      generator = next acc --urmatorul pair [numar - generator]
      urm = snd generator --iau generatorul
      nr = fst generator --iau numarul
    in
      (fromInteger (toInteger nr)) : (randoms1 seed urm)
...
--wordToAlpha n = chr (fromIntegral (toInteger n)) 

wordToAlpha n =
	let 
		nr = mod (fromIntegral (toInteger n)) 26
		ascii_a = ord 'a'
		toConvert = nr + ascii_a
	in 
		chr toConvert
...
--randomAlphaKey n = map wordToAlpha (randoms n) Fara point-free
randomAlphaKey = (map wordToAlpha) . (randoms) --randoms il ia pe n si rezultatu e pasata lui (map wordToAlpha)
{-
  6. (3p)
...
tableToFunc table char  =
  let
    tup = filter (search char) table
  in
    snd (head tup)

substCrypt :: [(Char, Char)] -> String -> String
substCrypt = (map . tableToFunc)
--substCrypt table list = map (tableToFunc table) list --fara point free
{-
    7. (Bonus - 1p)
