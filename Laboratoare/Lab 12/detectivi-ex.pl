...
%Verii lui X sunt copiii fratilor/surorilor parintilor lui X

%veri(X, V) :-
%    findall(Var, (c(P,X), c(G,P), c(G, A), c(A , Var), A \= P, Var \= X), V1), sort(V1,V).
...
%Fratii lui X sunt copiii Parintilor lui X.
frati(X, F) :-
    findall(Bro, (c(P,X), c(P,Bro), Bro \= X), F1), sort(F1,F).
...
x(L,A,B,N) :-
    findall(X,(member(X,L), (X > A), (X < B)), N).


mini(L,M) :-
    member(M,L), forall(member(N,L), N >= M).

factorial2(0,1).
factorial2(N, F) :- factorial2(N1,F1), N is N1 + 1, F is F1 *N.

minmax(L, Min, Max) :-
    member(Min,L), member(Max, L), forall(member(N,L), (N >= Min, Max >= N)).
   
gen(L,0,X).
gen([H|T],N,X) :-
    gen(T1,N1,X1), T1 is T|H, N is N1 - 1, X is H|X1.    
...
suspect(_Nume:_Marca:_Arma) :- inarmat(_Nume, _Arma), conduce(_Nume,_Marca).
...
au_arma(_Arma, _ListaNume) :- bagof(X,inarmat(X, _Arma), _ListaNume).

au_marca(_Marca, _ListaNume) :- bagof(X,conduce(X, _Marca), _ListaNume).
...
arme_bmw(_ListaArme) :- findall(X, ((inarmat(Nume, X)), (conduce(Nume, bmw))), _ListaArme).
...
arme_marca(_Marca, _ListaArme) :- findall(X, ((inarmat(Nume, X)), (conduce(Nume, _Marca))), _ListaArme).
