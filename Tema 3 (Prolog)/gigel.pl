...
match_rule([], _, rule([], _, _, _, _)).
match_rule([HeadToken|TailTokens], UserMemory, rule([HeadToken|TailWords], _, _, _, _)) :- match_rule(TailTokens, UserMemory, rule(TailWords, _, _, _, _)).
...
find_matching_rules(Tokens, Rules, _, MatchingRules) :-
    %iau toate regulile care fac match cu Tokens
...
%scoate replicile dintr-un rule
get_lines(rule(_, L, _, _, _), L).

%scoate actiunile dintr-un rule
get_actions(rule(_, _, A, _, _), A).

select_answer(Tokens, UserMemory, BotMemory, Answer, Actions) :- 
    %Bag intr-o lista toate regulile din fiecare rules care au cuvantul
    %cheie in tokens, prin constructor, unde rule(cuv_cheie, rules),
    %si iau rules pentru care cuv_cheie apartine tokens
    findall(X, (rules(Y,X), ord_subset(Y,Tokens)), RulesFiltered),
    %Fac flatten pe replicile rezultate ca primesc lista de liste
    flatten(RulesFiltered, AllRules),
    %Iau regulile care fac match
    find_matching_rules(Tokens, AllRules, UserMemory, MatchedRules),
    %Iau prima regula matchuita
    get_head(MatchedRules, FirstRule),
    %Iau toate raspunsurile
    get_lines(FirstRule, Replici),
    %Iau toate actiunile
    get_actions(FirstRule, Actions),
    %Fac dictionar de forma [(replica,scor)] din toate replicile
    findall((X,Y),(member(X, Replici), get_answer(X, BotMemory, Y)), Dicitionar),
    %Folosesc min_element sa scot minimu din dictionar
    min_element(Dicitionar, Answer).
