...
data Cell = VerPipe | EmptyCell
instance Show Cell where
    show VerPipe = [verPipe]
    show EmptyCell = [emptyCell]

functionala :: [Char] -> ((Int, Int), Cell) -> [Char]
functionala acc element =
    if (snd (fst element) == 0) then
        (acc ++ "\n" ++ (show (snd element)))
    else (acc ++ (show  (snd element)))

let change (x,y) vec = vec//[((x,y), VerPipe) | x < (fst (snd (bounds vec))), y < (snd (snd (bounds vec)))] 
